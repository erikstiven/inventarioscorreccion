...
function genera_formulario_pedido($tmp = 0, $sAccion = 'nuevo', $aForm = '')
{
    ...
    switch ($sAccion) {
        case 'nuevo':
            ...
            $ifu->AgregarCampoTexto('cliente_nombre', 'Proveedor|left', true, '', 250, 200);
    ...
    $sHtml_cab .= '<div class="col-md-12">
                        <div class="form-row">
                            <div class="col-md-3">
                                <label class="control-label" for="cliente_nombre">* Proveedor:</label>                                
                                <div class="input-group">
                                    <input type="text" class="form-control input-sm" placeholder="ESCRIBA PROVEEDOR Y PRESIONE ENTER" id="cliente_nombre" 
                        ...
                        <div class="form-row">
                            ...
                            <div class="col-md-2">
                                <label for="fecha_retencion" class="control-label">Fecha Registro Retencion:</label>      
                                    <input type="date" name="fecha_retencion" id="fecha_retencion" step="1" value="" class="form-control input-sm"  >
                            </div>  
                            <div class="col-md-1">
    ...
    $sHtml_lista_compras .= '<div class="col-md-12">
                        <div class="form-row">
                            <div class="col-md-3">
                                <label class="control-label" for="cliente_nombre_listac">* Proveedor:</label>                                
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control input-sm" placeholder="ESCRIBA PROVEEDOR Y PRESIONE ENTER" id="cliente_nombre_listac" 
    ...
    // ---------------------------------------------------------------------------------------
    //  CLAVE ACCESO COMPRAS ELECTRONICAS
    // ---------------------------------------------------------------------------------------
    ...
    if (!empty($clave_acceso)) {
        ...
        $oReturn->script("valida_existe_factura()");
    }

    // ---------------------------------------------------------------------------------------
    //  FIN CLAVE ACCESO COMPRAS ELECTRONICAS
    // ---------------------------------------------------------------------------------------
    ...
    if ($oIfx->Query($sql_saeminv)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                ...
                $minv_aut_usua = $oIfx->f('minv_aut_usua');
                $minv_ser_docu = $oIfx->f('minv_ser_docu');
                $minv_fac_prov = $oIfx->f('minv_fac_prov');
                $minv_cm1_minv = $oIfx->f('minv_cm1_minv');
                $minv_fec_entr = $oIfx->f('minv_fec_entr');
                $minv_fec_valo = $oIfx->f('minv_fec_valo');
                $minv_cod_mone = $oIfx->f('minv_cod_mone');
                $minv_val_tcam = $oIfx->f('minv_val_tcam');
                $minv_fmov = $oIfx->f('minv_fmov');

                $numero_digitos = strlen($minv_aut_usua);
                if ($numero_digitos > 15) {
                    $tipo_factura = 1; //Electronica
                } else {
                    $tipo_factura = 2; //Preimpresa
                }

                // DATOS PROVEEDOOR
                $minv_cod_clpv = $oIfx->f('minv_cod_clpv');
                $sql_nombre_clpv = "SELECT 
                                        clpv_cod_clpv, clpv_nom_clpv, clpv_ruc_clpv,
                                        clpv_cod_fpagop, clpv_cod_tpago
                                    from saeclpv WHERE clpv_cod_clpv = $minv_cod_clpv limit 1;";
                if ($oIfxA->Query($sql_nombre_clpv)) {
                    if ($oIfxA->NumFilas() > 0) {
                        do {
                            ...
                            $clpv_cod_fpagop = $oIfxA->f('clpv_cod_fpagop');
                            $clpv_cod_tpago = $oIfxA->f('clpv_cod_tpago');
                ...
                $oReturn->assign("tipo_factura", "value", $tipo_factura);
                $oReturn->script("cargar_factura();");
                $oReturn->assign("observaciones", "value", $minv_cm1_minv);
                $oReturn->assign("fecha_pedido", "value", $minv_fmov);
                $oReturn->assign("fecha_inicio", "value", $minv_fmov);
                $oReturn->assign("fecha_entrega", "value", $minv_fec_entr);
                $oReturn->assign("fecha_final", "value", $minv_fec_entr);
                $oReturn->assign("moneda", "value", $minv_cod_mone);
                $oReturn->assign("cotizacion", "value", $minv_val_tcam);


                $dias = (strtotime($minv_fmov) - strtotime($minv_fec_entr)) / 86400;
                $dias = abs($dias);
                $dias = floor($dias);
                $oReturn->assign("plazo", "value", $dias);
                ...
                // -------------------------------------------------------------------------------------------------------------------
                // Cargar productos dentro del inventario compra
                // -------------------------------------------------------------------------------------------------------------------
                ...
                $sql_data_dmov = "SELECT d.dmov_cod_prod, d.dmov_cod_bode, d.dmov_cod_unid,  
                                (d.dmov_can_dmov - d.dmov_can_entr) as cantidad, 
                                p.prbo_cta_inv, p.prbo_cta_ideb, p.prbo_iva_porc, d.dmov_cun_dmov, dmov_det1_dmov,
                                pr.prod_nom_prod, d.dmov_cod_lote, d.dmov_ela_lote, d.dmov_cad_lote,
                                d.dmov_cod_serie, d.dmov_iva_porc, d.dmov_ds1_dmov,
                                d.dmov_ds2_dmov, d.dmov_des_tota, d.dmov_cod_ccos
                                from saedmov d , saeprbo p, saeprod pr where
                                p.prbo_cod_prod = d.dmov_cod_prod and
                                p.prbo_cod_prod = pr.prod_cod_prod and
                                d.dmov_cod_prod = pr.prod_cod_prod and
                                d.dmov_cod_sucu = pr.prod_cod_sucu and
                                d.dmov_cod_bode = p.prbo_cod_bode and
                                d.dmov_num_comp = $num_comp_edit
                                order by d.dmov_cod_dmov ";

                if ($oIfxA->Query($sql_data_dmov)) {
                    if ($oIfxA->NumFilas() > 0) {
                        do {
                            $cont = count($aDataGrid);
                            $prod_cod = $oIfxA->f('dmov_cod_prod');
                            $bode_cod = $oIfxA->f('dmov_cod_bode');
                            $unid_cod = $oIfxA->f('dmov_cod_unid');
                            $cantidad = round($oIfxA->f('cantidad'), 4);
                            $costo = number_format($oIfxA->f('dmov_cun_dmov'), 4);
                            $cta_inv = $oIfxA->f('prbo_cta_inv');
                            $cta_iva = $oIfxA->f('prbo_cta_ideb');
                            $iva = $oIfxA->f('dmov_iva_porc');
                            $detalle = $oIfxA->f('dmov_det1_dmov');
                            $prod_nom_prod = $oIfxA->f('prod_nom_prod');
                            $dmov_cod_lote = $oIfxA->f('dmov_cod_lote');
                            $dmov_ela_lote = $oIfxA->f('dmov_ela_lote');
                            $dmov_cad_lote = $oIfxA->f('dmov_cad_lote');
                            $dmov_cod_serie = $oIfxA->f('dmov_cod_serie');
                            $dmov_iva_porc = $oIfxA->f('dmov_iva_porc');
                            $dmov_cod_ccos = $oIfxA->f('dmov_cod_ccos');

                            $dmov_ds1_dmov = number_format($oIfxA->f('dmov_ds1_dmov'), 2);
                            $dmov_ds2_dmov = number_format($oIfxA->f('dmov_ds2_dmov'), 2);
                            $dmov_des_tota = number_format($oIfxA->f('dmov_des_tota'), 2);


                            // TOTAL
                            $total_fac = 0;
                            $descuento = $dmov_ds1_dmov;
                            $descuento_2 = $dmov_ds2_dmov;
                            $descuento_general = $dmov_des_tota;
                            ...
                            $sql_prbo_cuentas = "SELECT prbo_cta_inv, prbo_cta_ideb, prbo_cod_unid, COALESCE(prbo_iva_porc,0) as prbo_iva_porc from saeprbo where prbo_cod_prod = '$prod_cod' and prbo_cod_sucu = $sucursal";
                            if ($oIfxB->Query($sql_prbo_cuentas)) {
                                if ($oIfxB->NumFilas() > 0) {
                                    do {
                                        $prbo_cta_inv = $oIfxB->f('prbo_cta_inv');
                                        $prbo_cta_ideb = $oIfxB->f('prbo_cta_ideb');
                            ...
                            $aDataGrid[$cont][$aLabelGrid[3]] = $prod_nom_prod;
                            ...
                            $aDataGrid[$cont][$aLabelGrid[13]] = $dmov_cod_lote;
                            $aDataGrid[$cont][$aLabelGrid[14]] = $dmov_ela_lote;
                            $aDataGrid[$cont][$aLabelGrid[15]] = $dmov_cad_lote;
                            ...
                            $aDataGrid[$cont][$aLabelGrid[23]] = $dmov_cod_lote;
                            $aDataGrid[$cont]['ccosn'] = $dmov_cod_ccos;
                            ...
                            $aDataGrid[$cont][$aLabelGrid[26]] = $dmov_cod_serie;
                            $aDataGrid[$cont][$aLabelGrid[27]] = $dmov_cod_serie;
                ...
                // -------------------------------------------------------------------------------------------------------------------
                // FIN Cargar productos dentro del inventario compra
                // -------------------------------------------------------------------------------------------------------------------
...
function cargar_info_fact_ad($num_comp_edit, $aForm)
{
    //Definiciones
    global $DSN, $DSN_Ifx;
    if (session_status() !== PHP_SESSION_ACTIVE) {
    ...
    if ($oIfx->Query($sql_saeminv)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                ...
                $oReturn->assign("serie_prove", "value", $minv_ser_docu);
                $oReturn->assign("factura", "value", $minv_fac_prov);
                $oReturn->assign("auto_prove", "value", $minv_aut_usua);
            } while ($oIfx->SiguienteRegistro());
        }
    }
    ...
    // -------------------------------------------------------------------------------------------------------------------
    // Cargar Forma Pago Factura Compra
    // -------------------------------------------------------------------------------------------------------------------
    $sql_forma_pago = "SELECT mxfp_cod_fpag from saemxfp where mxfp_num_comp = $num_comp_edit;";
    if ($oIfx->Query($sql_forma_pago)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                $mxfp_cod_fpag = $oIfx->f('mxfp_cod_fpag');
                $oReturn->assign("forma_pago_prove", "value", $mxfp_cod_fpag);
            } while ($oIfx->SiguienteRegistro());
        }
    }
    $oReturn->script('anadir_detalle_fp(' . $sucursal . ');');

    // -------------------------------------------------------------------------------------------------------------------
    // FIN Cargar Forma Pago Factura Compra
    // -------------------------------------------------------------------------------------------------------------------


    // -------------------------------------------------------------------------------------------------------------------
    // Cargar descuento general dmov
    // -------------------------------------------------------------------------------------------------------------------
    $sql_descuento_general = "SELECT dmov_des_tota from saedmov where dmov_num_comp = $num_comp_edit;";
    if ($oIfx->Query($sql_descuento_general)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                $dmov_des_tota = $oIfx->f('dmov_des_tota');
                $oReturn->assign("descuento_general", "value", round($dmov_des_tota, 2));
            } while ($oIfx->SiguienteRegistro());
        }
    }
    $oReturn->script('totales();');
    // -------------------------------------------------------------------------------------------------------------------
    // FIN Cargar descuento general dmov
    // -------------------------------------------------------------------------------------------------------------------
        ...
        if ($oIfx->Query($sql)) {
            if ($oIfx->NumFilas() > 0) {
                ...
                do {
                    ...
                    $arrayCompras[] = array(
                        $fecha,
                        $oIfx->f('fprv_num_seri'),
                        $oIfx->f('fprv_num_fact'),
                        $oIfx->f('fprv_cod_asto'),
                        $tran,
                        $oIfx->f('clpv_nom_clpv'),
                        $oIfx->f('fprv_det_fprv'),
                        $oIfx->f('total_graba_12'),
                        $oIfx->f('total_graba_0'),
                        $oIfx->f('valor_iva'),
                        $oIfx->f('total'),
                        $oIfx->f('no_ojeto_iva'),
                        $oIfx->f('exento_iva'),
...
function valida_existe_factura($aForm = '', $clave)
{
    ...
    if ($count_existe_factura > 0) {
        ...
        $oReturn->script('validar_proveedor()');
    }
...
function validar_proveedor($aForm = '', $tipo, $leerxml, $codigoPrincipal)
{
    ...
    try {
        ...
        if ($estado[$pos] == 'AUTORIZADO') {
            ...
            $identificacionComprador = trim($xmlParse->infoFactura->identificacionComprador);

            if ($ruc <>  $identificacionComprador)
            {
                $mensaje = 'Esta Factura NO fue Emitida a esta Empresa';
                $oReturn->script('alerts("' . $mensaje . '");');
            }
            else
            {
                $sql = "SELECT clpv_cod_clpv 
                            from saeclpv where
                ...
                if ($cod_clpv > 0) {
                    ...
                    $oReturn->script('crear_proveedor()');
                }
            }
        }
    } catch (SoapFault $e) {
...
function formulario_etiqueta($id)
{
    ...
    $fu->AgregarCampoNumerico('ancho', 'Ancho(cm)|left', true, 0, 50, 50);
    $fu->AgregarCampoNumerico('alto', 'Alto(cm)|left', true, 0, 50, 50);
    ...
    if ($oIfx->Query($sql1)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                $arrayEtiqueta[] = array($prod_cod_prod, $marca, $nomcolor, $nomtalla, $ppr_pre_raun, $serial, $idempresa, $idsucursal, $prbo_cod_bode);
...
function guarda_pedido($opcion_tmp, $aForm = '')
{
    ...
    // variables validacion asiento descuadrado
    $debito_valida_descuadre = 0;
    $debito_valida_txt = '';
    $credito_valida_descuadre = 0;
    $credito_valida_txt = '';
    // FIN variables validacion asiento descuadrado
    ...
    if ($contdata > 0 && $contdatafp > 0 && $inv_ctrl == 1) {
        ...
        try {
            ...
            $oIfxB->QueryT('BEGIN WORK;');
            $oIfxC->QueryT('BEGIN WORK;');
            ...
            $fecha_pedido     = $aForm['fecha_pedido'];  // FECHA REGISTRO CONTABLE
            $fecha_entrega     = $aForm['fecha_entrega']; // FECHA PAGO
            $fecha_regc         = $aForm['fecha_regc']; // FECHA COMPRA
            ...
            if ($contador_ > 0 && $contador_1 > 0) {
                ...
                if ($controles) {
                    ...
                    $oIfx->QueryT($sql);
                    $credito_valida_descuadre = $credito_valida_descuadre + $total_compra;
                    $credito_valida_txt = $credito_valida_txt . ' - ' . $total_compra;
                    ...
                    $total_fact_actual = ($fact_tot + $iva_total) - $desc_valor;
                    ...
                    $sql_minv = "insert into saeminv(	minv_num_comp,      minv_num_plaz,  	minv_num_sec,       	minv_cod_tcam,
														...
														minv_ruc_clpv,      minv_comp_ord,          minv_fec_ret
                                                        )
                                                  values( ($minv_num_comp + 1),   1,             	'$secu_minv',       	$tcambio,
                                                        ...
                                                        '$minv_ruc_clpv'	, $serial_oc,           $fecha_retencion
                                                        ) ";
                    ...
                    if (count($array_otros) > 0) {
                        ...
                        foreach ($array_otros as $val) {
                            ...
                            if ($val_txt > 0) {
                                ...
                                if (empty($cuen_nom)) {
                                    throw new Exception('La cuenta contable: ' . $cuen_otro . ' de OTROS, No existe. ...');
                                }
                                ...
                                $oIfx->QueryT($sql);
                                $debito_valida_descuadre = $debito_valida_descuadre + $val_txt;
                                $debito_valida_txt = $debito_valida_txt . ' 1 - ' . $val_txt;
                        ...
                        if (count($array_codigos_unicos_rec) > 0 && $producto_config_serie_auto == 'S') {
                            foreach ($array_codigos_unicos_rec as $key49 => $codigo_unico) {
                                if (!empty($codigo_unico)) {
                                    ...
                                    $stock_sumado_movimiento = $stock + $cant;

                                    $dmov_can_dmov_prom = $cant;
                                    $dmov_pun_dmov_prom = $costo_real;
                                    $dmov_pto_dmov_prom = $dmov_can_dmov_prom * $dmov_pun_dmov_prom;
                                    $sql_costo_prom = "SELECT 
                                                                    dmov_fmov, 
                                                                    dmov_cost_pro, 
                                                                    dmov_nuev_sal, 
                                                                    dmov_hor_crea, 
                                                                ...
                                                                from 
                                                                    saedmov 
                                                                    inner join saeminv
                                                                        on minv_num_comp = dmov_num_comp
                                                                    inner join saetran
                                                                        on tran_cod_tran = minv_cod_tran
                                                                        and tran_cod_tran = dmov_cod_tran
                                                                        and minv_cod_empr = tran_cod_empr
                                                                    inner join saedefi
                                                                        on tran_cod_tran = defi_cod_tran 
                                                                        AND tran_cod_empr = defi_cod_empr 
                                                                        AND tran_cod_modu = defi_cod_modu 
                                                                    inner join saeprod
                                                                        on prod_cod_prod = dmov_cod_prod
                                                                where 
                                                                    dmov_cod_empr = $idempresa
                                                                    and dmov_cod_sucu = $sucursal
                                                                    and dmov_cod_bode = $bod
                                                                    and dmov_cod_prod = '$prod' 
                                                                    and dmov_cod_tran not in ('OCO', 'OCI') 
                                                                    and dmov_can_dmov > 0 
                                                                    and dmov_cost_pro > 0
                                                                    AND prod_cod_tpro = '2' 
                                                                order by 
                                                                ...
                                                                ;
                                                                ";
                                    $dmov_nuev_sal_movi = 0;
                                    $dmov_cost_pro_movi = 0;
                                    if ($oIfx->Query($sql_costo_prom)) {
                                        if ($oIfx->NumFilas() > 0) {
                                            do {
                                                $dmov_cost_pro = $oIfx->f('dmov_cost_pro');
                                                $dmov_nuev_sal = $oIfx->f('dmov_nuev_sal');
                                                $dmov_nuev_sal_movi = $dmov_nuev_sal + $dmov_pto_dmov_prom;
                                                $dmov_cost_pro_movi = $dmov_nuev_sal_movi / $stock_sumado_movimiento;
                                            } while ($oIfx->SiguienteRegistro());
                                        }
                                    }
                                    $oIfx->Free();

                                    if ($dmov_cost_pro_movi > 0) {
                                        $costo_real = $dmov_cost_pro_movi;

                                        $sql_update_dmov_costo_prom = "UPDATE 
                                                                                    saedmov 
                                                                                set 
                                                                                    dmov_cost_pro = $dmov_cost_pro_movi,
                                                                                    dmov_nuev_sal = $dmov_nuev_sal_movi
                                                                                where
                                                                                    dmov_nun_comp = $serial_minv and
                                                                                    dmov_cod_dmov = $x and
                                                                                    dmov_cod_bode = $bod and
                                                                                    dmov_cod_prod = '$prod' 
                                                                                ";
                                        $oIfx->QueryT($sql_update_dmov_costo_prom);
                                    }
                            ...
                            foreach ($aValues as $key => $aVal) {
                                ...
                                if ($aux == 0) {
                                    ...
                                    // ARRAY DE CUENTA PROD Y IVA
                                    if ($iva > 0) {
                                        $iva_total_ad = $total * $iva / 100;
                                    } else {
                                        $iva_total_ad = 0;
                                    }
                                    $arrray_dmov[$cuenta_prod] += $total;
                                    $arrray_dmov[$cuenta_iva] += $iva_total_ad;

                                    //var_dump($total . ' - ' . $iva . ' - ' . $iva_total_ad);
                                    //var_dump($arrray_dmov);
                                    //exit;
                    ...
                    if (count($arrray_dmov) > 0) {
                        $suma = 0;
                        $contador_dasi_prod = 1;
                        $array_data_dasi_descuadre = array();
                        foreach (array_keys($arrray_dmov) as $key) {
                            ...
                            $oIfx->QueryT($sql);
                            $debito_valida_descuadre = $debito_valida_descuadre + $suma;
                            $debito_valida_txt = $debito_valida_txt . ' 2 - ' . $suma;
                            ...
                            if ($contador_dasi_prod == 1) {
                                $data_dasi = array(
                                    "asto_cod_asto" => $secu_asto,
                                    "asto_cod_empr" => $idempresa,
                                    "asto_cod_sucu" => $sucursal,
                                    "dasi_num_prdo" => $idprdo,
                                    "asto_cod_ejer" => $idejer,
                                    "dasi_cod_cuen" => $key,
                                    "dasi_dml_dasi" => $suma,
                                    "dasi_cml_dasi" => 0,
                                    "dasi_dme_dasi" => $suma_ext,
                                    "dasi_cme_dasi" => 0,
                                    "dasi_tip_camb" => $coti,
                                    "dasi_det_asi" => $detalle_asto,
                                    "dasi_nom_ctac" => $cuen_prod_nom,
                                    "dasi_cod_clie" => $cliente,
                                    "dasi_cod_tran" => $tran,
                                    "dasi_user_web" => $usuario_web
                                );
                                array_push($array_data_dasi_descuadre, $data_dasi);
                            }
                    ...
                    if (!empty($minv_cod_edit)) {
                        ...
                        $sql_total_fact_ant = "SELECT (COALESCE(minv_tot_minv, 0) + COALESCE(minv_iva_valo, 0)) - COALESCE(minv_dge_valo, 0) as total_factura, minv_comp_cont, minv_fmov from saeminv where minv_num_comp = $minv_cod_edit;";
                        $total_factura_ant = consulta_string($sql_total_fact_ant, 'total_factura', $oIfx, 0);
                        $minv_comp_cont = consulta_string($sql_total_fact_ant, 'minv_comp_cont', $oIfx, 0);
                        $minv_fmov = consulta_string($sql_total_fact_ant, 'minv_fmov', $oIfx, 0);

                        if (round($total_factura_ant, 2) == round($total_fact_actual, 2)) {
                            $inserta_retencion = 'N';
                            ...
                            $fecha_ejer = $anio . '-12-31';
                            $sql_ejer = "select ejer_cod_ejer from saeejer where ejer_fec_finl = '$fecha_ejer' and ejer_cod_empr = $idempresa";
                            $idejer = consulta_string($sql_ejer, 'ejer_cod_ejer', $oIfx, 1);
                            $idprdo = (substr($minv_fmov, 5, 2)) * 1;
                            $num_ret = $aForm['ret_num'];
                            $total_ret = 0;

                            $sql_retencion = "SELECT * from saeret where rete_cod_asto = '$minv_comp_cont' and asto_cod_ejer = $idejer and asto_num_prdo = $idprdo;";
                            if ($oIfxA->Query($sql_retencion)) {
                                if ($oIfxA->NumFilas() > 0) {
                                    do {
                                        $ret_cod_ret = $oIfxA->f('ret_cod_ret');
                                        $ret_num_ret = $oIfxA->f('ret_num_ret');
                                        $ret_ser_ret = $oIfxA->f('ret_ser_ret');
                                        $ret_aut_ret = $oIfxA->f('ret_aut_ret');
                                        $ret_elec_sn = $oIfxA->f('ret_elec_sn');
                                        $ret_fec_ret = $oIfxA->f('ret_fec_ret');
                                        $ret_valor = $oIfxA->f('ret_valor');

                                        $ret_cta_ret = $oIfxA->f('ret_cta_ret');
                                        $ret_porc_ret = $oIfxA->f('ret_porc_ret');
                                        $ret_bas_imp = $oIfxA->f('ret_bas_imp');

                                        if (!empty($ret_cta_ret)) {
                                            // SAEDASI RETENCION
                                            // NOMBRE CUENTA PRODUCTOS

                                            $sql_cuenta_retecnion = "SELECT tret_cta_deb, tret_cta_cre from saetret where
                                                            tret_cod_empr = $idempresa and
                                                            tret_cod = '$ret_cta_ret' ";
                                            $tret_cuen = consulta_string($sql_cuenta_retecnion, 'tret_cta_deb', $oIfx, '');
                                            if (empty($tret_cta_deb)) {
                                                $tret_cuen = consulta_string($sql_cuenta_retecnion, 'tret_cta_cre', $oIfx, '');
                                            }

                                            $sql = "select cuen_nom_cuen from saecuen where
                                                            cuen_cod_empr = $idempresa and
                                                            cuen_cod_cuen = '$tret_cuen' ";
                                            $cuen_ret_nom = consulta_string($sql, 'cuen_nom_cuen', $oIfx, '');


                                            if (empty($cuen_ret_nom)) {
                                                throw new Exception('La cuenta contable: ' . $tret_cuen . ', No existe. Verifica las cuentas contables de la retencion ...');
                                            }

                                            //$secu_asto
                                            $sql_update_ret  = "UPDATE saeret 
                                                                        set 
                                                                            rete_cod_asto = '$secu_asto'
                                                                        where 
                                                                            rete_cod_asto = '$minv_comp_cont'
                                                                            and asto_cod_ejer = $idejer
                                                                            and asto_num_prdo = $idprdo
                                                                            --and ret_cod_ret = $ret_cod_ret
                                                                    ";
                                            $oIfx->QueryT($sql_update_ret);
                                            ...
                                            if ($moneda == $mone_base) {
                                                ...
                                                $val_camb = consulta_string_func($sql, 'tcam_val_tcam', $oIfx, '0');
                                                $val_ret_ext = round(($val_ret_ext / $val_camb), 2);
                                            } else {
                                                $val_ret_ext = round(($val_ret_ext / $coti), 2);
                                            }
                                            ...
                                            $sql = "insert into saedasi (asto_cod_asto, asto_cod_empr, asto_cod_sucu, dasi_num_prdo,
                                                                    ...
                                                                    values ( '$secu_asto', $idempresa, $sucursal, $idprdo,
                                                                    $idejer, '$ret_cta_ret', 0, $ret_valor,
                                                                    0, $val_ret_ext, $coti, '$detalle_asto' ,
                                                                    '$cuen_ret_nom', '$num_ret', '$ret_cta_ret', $cliente,
                                                                    '$tran', $usuario_web ); ";
                                            $oIfx->QueryT($sql);

                                            $credito_valida_descuadre = $credito_valida_descuadre + $ret_valor;
                                            $credito_valida_txt = $credito_valida_txt . ' - ' . $ret_valor;
                                            $total_ret += $ret_valor;
                                            ...
                                            if ($codigo_pais == 51 && $pcon_cue_niif == 'S') {
                                                ...
                                                // Cuando es Peru baja dos cuentas adicionales: cuan_ana_deb y cuen_anal_cre
                                                $sql_cuen = "SELECT cuen_ana_deb, cuen_ana_cre from saecuen where cuen_cod_cuen = '$ret_cta_ret';";
                                                $cuen_ana_deb = consulta_string_func($sql_cuen, 'cuen_ana_deb', $oIfx, '');
                                                $cuen_ana_cre = consulta_string_func($sql_cuen, 'cuen_ana_cre', $oIfx, '');
                            ...
                            if ($ret_asumido == 'S') {
                                ...
                                if ($moneda != $mone_base) {
                                    $cotizacion_ad = $aForm["cotizacion"];
                                    $total_ret = round($total_ret * $cotizacion_ad, 2);
                                }
                                // -------------------------------------------------------------------------------------------------------------------------------------
                                // FIN Metodo para recepcionar la compra en moneda secundaria Adrian47
                                // -------------------------------------------------------------------------------------------------------------------------------------
                                ...
                                if (empty($cuen_nom_cuen)) {
                                    throw new Exception('La cuenta contable: ' . $defi_cret_asumi . ', No existe. Verifica las cuentas contables de la retencion ...');
                                }
                                ...
                                $sql = "insert into saedasi ( asto_cod_asto, asto_cod_empr, asto_cod_sucu, dasi_num_prdo,
                                            ...
                                            '$num_ret' ); ";
                                $oIfx->QueryT($sql);
                                $debito_valida_descuadre = $debito_valida_descuadre + $total_ret;
                                $debito_valida_txt = $debito_valida_txt . ' 3 - ' . $total_ret;
                                ...
                                if ($codigo_pais == 51 && $pcon_cue_niif == 'S') {
                                    ...
                                    // Cuando es Peru baja dos cuentas adicionales: cuan_ana_deb y cuen_anal_cre
                                    $sql_cuen = "SELECT cuen_ana_deb, cuen_ana_cre from saecuen where cuen_cod_cuen = '$defi_cret_asumi';";
                                    $cuen_ana_deb = consulta_string_func($sql_cuen, 'cuen_ana_deb', $oIfx, '');
                                    $cuen_ana_cre = consulta_string_func($sql_cuen, 'cuen_ana_cre', $oIfx, '');
                                ...
                                if (empty($cuen_prove_nom)) {
                                    throw new Exception('La cuenta contable: ' . $cuenta_prove . ' de PROVEEDORES, No existe. ...');
                                }
                                ...
                                $oIfx->QueryT($sql);
                                $debito_valida_descuadre = $debito_valida_descuadre + $total_ret;
                                $debito_valida_txt = $debito_valida_txt . ' 4 - ' . $total_ret;
                        ...
                        $sql_anular_minv = "SELECT * FROM saeminv where minv_num_comp = $minv_cod_edit";
                        if ($oIfx->Query($sql_anular_minv)) {
                            if ($oIfx->NumFilas() > 0) {
                                do {
                                    $minv_cod_empr = $oIfx->f('minv_cod_empr');
                                    ...
                                    $minv_cod_tran  = $oIfx->f('minv_cod_tran');
                                    $minv_cod_clpv  = $oIfx->f('minv_cod_clpv');
                                    //eliminar_movimiento($aForm = '', $num_comp, $cod_empresa, $cod_sucursal, $cod_asto, $cod_prdo, $cod_ejer, $cod_tran, $minv_cod_clpv)
                                    $oReturn->script("eliminar_movimiento($minv_cod_edit, $minv_cod_empr, $minv_cod_sucu, $minv_comp_cont, $minv_num_prdo, $minv_cod_ejer, $minv_cod_tran, $minv_cod_clpv)");
                                } while ($oIfx->SiguienteRegistro());
                            }
                        }
                        $oIfx->Free();
                    ...
                    if (count($aDataGrid_Rete) > 0 &&  $inserta_retencion == 'S') {
                        ...
                        foreach ($aDataGrid_Rete as $aValues) {
                            ...
                            foreach ($aValues as $aVal) {
                                if ($aux == 0) {
                                    ...
                                    if ($moneda != $mone_base) {
                                        $cotizacion_ad = $aForm["cotizacion"];
                                        $val_ret = round($val_ret * $cotizacion_ad, 2);
                                        $base_impo = round($base_impo * $cotizacion_ad, 2);
                                    }
                                    // -------------------------------------------------------------------------------------------------------------------------------------
                                    // FIN Metodo para recepcionar la compra en moneda secundaria Adrian47
                                    // -------------------------------------------------------------------------------------------------------------------------------------
                                    ...
                                    if (!empty($cod_ret)) {
                                        ...
                                        if (empty($cuen_ret_nom)) {
                                            throw new Exception('La cuenta contable: ' . $cta_re . ', No existe. Verifica las cuentas contables de la retencion ...');
                                        }
                                        ...
                                        $sql = "insert into saedasi (asto_cod_asto,        asto_cod_empr,      asto_cod_sucu,      dasi_num_prdo,
                                                                                    ...
                                                                                    values  ( '$secu_asto',         $idempresa,         $sucursal,          $idprdo,
                                                                                              ...
                                                                                              '$tran',            $usuario_web   ); ";
                                        $oIfx->QueryT($sql);
                                        $credito_valida_descuadre = $credito_valida_descuadre + $val_ret;
                                        $credito_valida_txt = $credito_valida_txt . ' - ' . $val_ret;
                                        $total_ret += $val_ret;

                                        //throw new Exception($codigo_pais);
                                        ...
                                        if ($codigo_pais == 51 && $pcon_cue_niif == 'S') {
                                            ...
                                            // Cuando es Peru baja dos cuentas adicionales: cuan_ana_deb y cuen_anal_cre
                                            $sql_cuen = "SELECT cuen_ana_deb, cuen_ana_cre from saecuen where cuen_cod_cuen = '$cta_re';";
                                            $cuen_ana_deb = consulta_string_func($sql_cuen, 'cuen_ana_deb', $oIfx, '');
                                            $cuen_ana_cre = consulta_string_func($sql_cuen, 'cuen_ana_cre', $oIfx, '');
                        ...
                        if ($ret_asumido == 'S') {
                            ...
                            // Metodo para recepcionar la compra en moneda secundaria Adrian47
                            // -------------------------------------------------------------------------------------------------------------------------------------
                            // LA RETENCION INGRESA EN MONEDA LOCAL POR ESTO ES QUE NO SE REALIZA LA VERIFICACION DE LA MONEDA NUEVAMENTE
                            /*
                            if ($moneda != $mone_base) {
                                $cotizacion_ad = $aForm["cotizacion"];
                                $total_ret = round($total_ret * $cotizacion_ad, 2);
                    ...
                    // -----------------------------------------------------------------------
                    // Validacion asiento descuadrado
                    // -----------------------------------------------------------------------
                    $calculo_cuenadre_0 = number_format($debito_valida_descuadre, 4, '.', '') - number_format($credito_valida_descuadre, 4, '.', '');
                    if ($calculo_cuenadre_0 != 0) {
                        $calculo_cuenadre_0  = round($calculo_cuenadre_0, 2);
                        if ($debito_valida_descuadre > $credito_valida_descuadre) {
                            if (isset($array_data_dasi_descuadre)) {
                                foreach ($array_data_dasi_descuadre as $key => $data_dasi) {
                                    ...
                                    */
                                    ...
                                    $ad_data_asto_cod_asto = $data_dasi['asto_cod_asto'];
                                    $ad_data_asto_cod_empr = $data_dasi['asto_cod_empr'];
                                    $ad_data_asto_cod_sucu = $data_dasi['asto_cod_sucu'];
                                    $ad_data_dasi_num_prdo = $data_dasi['dasi_num_prdo'];
                                    $ad_data_asto_cod_ejer = $data_dasi['asto_cod_ejer'];
                                    $ad_data_dasi_cod_cuen = $data_dasi['dasi_cod_cuen'];
                                    $ad_data_dasi_dml_dasi = $data_dasi['dasi_dml_dasi'];
                                    $ad_data_dasi_cml_dasi = $data_dasi['dasi_cml_dasi'];
                                    $ad_data_dasi_dme_dasi = $data_dasi['dasi_dme_dasi'];
                                    $ad_data_dasi_cme_dasi = $data_dasi['dasi_cme_dasi'];
                                    $ad_data_dasi_tip_camb = $data_dasi['dasi_tip_camb'];
                                    $ad_data_dasi_det_asi = $data_dasi['dasi_det_asi'];
                                    $ad_data_dasi_nom_ctac = $data_dasi['dasi_nom_ctac'];
                                    $ad_data_dasi_cod_clie = $data_dasi['dasi_cod_clie'];
                                    $ad_data_dasi_cod_tran = $data_dasi['dasi_cod_tran'];
                                    $ad_data_dasi_user_web = $data_dasi['dasi_user_web'];

                                    $sql_delete_dasi = "DELETE from 
                                                            saedasi 
                                                        where 
                                                            asto_cod_asto = '$ad_data_asto_cod_asto'
                                                            and asto_cod_empr = $ad_data_asto_cod_empr
                                                            and asto_cod_sucu = $ad_data_asto_cod_sucu
                                                            and dasi_num_prdo = $ad_data_dasi_num_prdo
                                                            and asto_cod_ejer = $ad_data_asto_cod_ejer
                                                            and dasi_cod_cuen = '$ad_data_dasi_cod_cuen'
                                                            and dasi_nom_ctac = '$ad_data_dasi_nom_ctac'
                                                            and dasi_cod_tran = '$ad_data_dasi_cod_tran'
                                                            and dasi_dml_dasi = $ad_data_dasi_dml_dasi
                                                            ";
                                    $oIfx->QueryT($sql_delete_dasi);

                                    $ad_data_dasi_dml_dasi = round($ad_data_dasi_dml_dasi - $calculo_cuenadre_0, 2);
                                    $sql = "INSERT into saedasi (asto_cod_asto,        asto_cod_empr,      asto_cod_sucu,      dasi_num_prdo,
                                                              asto_cod_ejer,        dasi_cod_cuen,      dasi_dml_dasi,      dasi_cml_dasi,
                                                              dasi_dme_dasi,        dasi_cme_dasi,      dasi_tip_camb,      dasi_det_asi,
                                                              dasi_nom_ctac,        dasi_cod_clie,      dasi_cod_tran,      dasi_user_web )
                                                    values  ( '$ad_data_asto_cod_asto', $ad_data_asto_cod_empr, $ad_data_asto_cod_sucu, $ad_data_dasi_num_prdo,
                                                               $ad_data_asto_cod_ejer, '$ad_data_dasi_cod_cuen', $ad_data_dasi_dml_dasi, 0,
                                                               $ad_data_dasi_dme_dasi, 0, $ad_data_dasi_tip_camb, '$ad_data_dasi_det_asi' ,
                                                              '$ad_data_dasi_nom_ctac', $ad_data_dasi_cod_clie, '$ad_data_dasi_cod_tran', $ad_data_dasi_user_web   ); ";
                                    $oIfx->QueryT($sql);

                                    $debito_valida_descuadre = $debito_valida_descuadre - $calculo_cuenadre_0;
                                }
                            }
                        }
                        $calculo_cuenadre_0_2 = number_format($debito_valida_descuadre, 4, '.', '') - number_format($credito_valida_descuadre, 4, '.', '');
                        if ($calculo_cuenadre_0_2 != 0) {
                            if (abs($calculo_cuenadre_0_2) > 0.01) {
                                throw new Exception('El asiento esta descuadrado por: ' . $calculo_cuenadre_0_2 . '. Valores DEBITO: ' . $debito_valida_descuadre . '(' . $debito_valida_txt . ') - CREDITO: ' . $credito_valida_descuadre . '(' . $credito_valida_txt . ')');
                            }
                        }
                    }

                    // -----------------------------------------------------------------------
                    // FIN Validacion asiento descuadrado
                    // -----------------------------------------------------------------------
                    ...
                    $oIfxB->QueryT('COMMIT WORK;');
                    $oIfxC->QueryT('COMMIT WORK;');
            ...
            $oIfxB->QueryT('ROLLBACK WORK;');
            $oIfxC->QueryT('ROLLBACK WORK;');
...
function eliminar_movimiento($aForm = '', $num_comp, $cod_empresa, $cod_sucursal, $cod_asto, $cod_prdo, $cod_ejer, $cod_tran, $minv_cod_clpv)
{
    global $DSN, $DSN_Ifx;
    session_start();
    ...
    $empresa = $aForm['empresa'];
    $sucursal = $aForm['sucursal'];
    $act_inv = $aForm['act_inv'];

    $array_productos_movimiento = array();
    ...
    try {
        ...
        // ----------------------------------------------------------------------------------------------
        // LIBERACION ORDEN DE COMPRA
        // ----------------------------------------------------------------------------------------------

        //VERIFICAMOS QUE LE MOVIMIENTO A ELIMINAR CONTENGA ORDEN DE COMPRA
        if (empty($minv_cod_clpv)) {
            $minv_cod_clpv = 0;
        }
        $sql_existe_oc = "SELECT min(minv_num_comp) as minv_num_comp_oc 
                            from saeminv 
                            where minv_cod_pedi in (select minv_cod_pedi from SAEMINV where minv_num_comp = '$num_comp')
                            and minv_cod_clpv = $minv_cod_clpv
                            ";
        $minv_num_comp_oc = consulta_string($sql_existe_oc, 'minv_num_comp_oc', $oIfx, 0);
        $array_productos_compra = array();
        if ($minv_num_comp_oc != 0) {

            // GUARDAMOS LOS PRODUCTOS Y CANTIDADES DEL MOVIMIENTO A ELIMINAR Y LOS ALMACENAMOS EN UN ARRAY
            // (Esto nos va a servir para actualizar la orden de compra con las cantidades anuladas)
            $sql_productos_compra = "SELECT dmov_cod_prod, dmov_can_dmov from saedmov where dmov_num_comp = '$num_comp'";

            if ($oIfx->Query($sql_productos_compra)) {
                if ($oIfx->NumFilas() > 0) {
                    do {

                        $dmov_cod_prod  = $oIfx->f('dmov_cod_prod');
                        $dmov_can = $oIfx->f('dmov_can_dmov');

                        $data = array(
                            "cod_prod" => $dmov_cod_prod,
                            "can_prod" => $dmov_can
                        );
                        array_push($array_productos_compra, $data);
                    } while ($oIfx->SiguienteRegistro());
                }
            }
        }
        ...
        // ----------------------------------------------------------------------------------------------
        // FIN LIBERACION ORDEN DE COMPRA
        // ----------------------------------------------------------------------------------------------

        if ($cod_asto == 0) {

            // ----------------------------------------------------------------------------------------------
            // ELIMINACION DE SAEDMOV
            // ----------------------------------------------------------------------------------------------

            $sql_insert_minv_an  = "INSERT into saedmov_an 
                                                select * from 
                                                saedmov
                                                where 
                                                dmov_num_comp = '$num_comp'
                                ";

            $oIfx->QueryT($sql_insert_minv_an);

            $sql_delete_dmov  = "UPDATE saedmov 
                                    set 
                                        dmov_can_dmov = 0, dmov_can_entr = 0, dmov_cun_dmov = 0,
                                        dmov_cto_dmov = 0, dmov_con_dmov = 0, dmov_pun_dmov = 0,
                                        dmov_pto_dmov = 0, dmov_ds1_dmov = 0, dmov_ds2_dmov = 0,
                                        dmov_ds3_dmov = 0, dmov_ds4_dmov = 0, dmov_des_tota = 0,
                                        dmov_imp_dmov = 0, dmov_iva_dmov = 0, dmov_ice_dmov = 0,
                                        dmov_pto1_dmov = 0, dmov_est_dmov = 0
									where 
									dmov_num_comp = '$num_comp'
								";
            $oIfx->QueryT($sql_delete_dmov);


            // ----------------------------------------------------------------------------------------------
            // ELIMINACION DE SAEMINV
            // ----------------------------------------------------------------------------------------------


            $sql_insert_minv_an  = "INSERT into saeminv_an 
                                                select * from 
                                                saeminv
                                                where 
            ...
            $sql_delete_minv  = "UPDATE saeminv 
                                    set 
                                        minv_est_minv = '0', minv_tot_minv = 0, minv_con_iva = 0,
                                        minv_sin_iva = 0, minv_iva_valo = 0, minv_otr_valo = 0,
                                        minv_fle_minv = 0, minv_imp_tot = 0, minv_dge_valo = 0
									where 
									minv_num_comp = '$num_comp'
								";
            $oIfx->QueryT($sql_delete_minv);

            // ----------------------------------------------------------------------------------------------
            //INGRESO TABLA AUDITORIA
            // ----------------------------------------------------------------------------------------------
            ...
            $sql_insert_uminv  = "INSERT INTO saeuminv_an (
                                                uminv_cod_empr, uminv_cod_sucu, uminv_cod_ejer, 
                                                uminv_cod_asto, uminv_num_comp, uminv_usua_uminv, 
                                                uminv_fec_ingre, uminv_usua_rest, uminv_fec_resta, 
                                                uminv_est_uminv )
                                                values (
                                                $cod_empresa, $cod_sucursal, $cod_ejer,
                                                $cod_asto, $num_comp, $id_usuario,
                                                '$fecha_actual', null, null, 'AN'
                                                );        
                                            ";
            $oIfx->QueryT($sql_insert_uminv);
        } else {

            // Aqui vemos si el numero conta tiene  valor significa que tiene movimeintos contables y debemos borrar mas tablas
            // ----------------------------------------------------------------------------------------------
            // ELIMINACION DE SAEDASI
            // ----------------------------------------------------------------------------------------------
            $sql_insert_dasi_an  = "INSERT into saedasi_an 
                                                select * from 
                                                saedasi 
                                                where 
                                                asto_cod_asto = '$cod_asto' and 
                                                asto_cod_empr = $cod_empresa and 
                                                asto_cod_sucu = $cod_sucursal and 
                                                asto_cod_ejer = $cod_ejer
                                             
                                ";
            $oIfx->QueryT($sql_insert_dasi_an);


            $sql_delete_dasi  = "UPDATE saedasi set dasi_dml_dasi=0.00,dasi_cml_dasi=0.00,
                                    dasi_dme_dasi=0.00,dasi_cme_dasi=0.00
                                    where 
                                    asto_cod_asto = '$cod_asto' and 
                                    asto_cod_empr = $cod_empresa and 
                                    asto_cod_sucu = $cod_sucursal and 
            ...
            // ----------------------------------------------------------------------------------------------
            // ELIMINACION DE SAEDMCP
            // ----------------------------------------------------------------------------------------------


            $sql_insert_dmcp_an  = "INSERT into saedmcp_an 
                                                select * from 
                                                saedmcp 
                                                where dmcp_cod_asto = '$cod_asto' and 
                                                dmcp_cod_empr = $cod_empresa and 
                                                dmcp_cod_sucu = $cod_sucursal and 
                                                dmcp_cod_ejer = $cod_ejer
                                             
                                ";
            $oIfx->QueryT($sql_insert_dmcp_an);

            $sql_delete_dmcp  = "UPDATE saedmcp set dmcp_mon_ml=0.00,dmcp_mon_ext=0.00,dcmp_deb_ml=0.00,
                                    dcmp_cre_ml=0.00,dmcp_deb_mext=0.00,dmcp_cre_mext=0.00 
                                    where dmcp_cod_asto = '$cod_asto' and 
                                    dmcp_cod_empr = $cod_empresa and 
                                    dmcp_cod_sucu = $cod_sucursal and 
                                    dmcp_cod_ejer = $cod_ejer";
            $oIfx->QueryT($sql_delete_dmcp);

            // ----------------------------------------------------------------------------------------------
            // ELIMINACION DE SAEDIR
            // ----------------------------------------------------------------------------------------------

            $sql_insert_dir_an  = "INSERT into saedir_an 
                                                select * from 
                                                saedir 
                                                where dire_cod_asto = '$cod_asto' and 
                                                dire_cod_empr = $cod_empresa and 
                                                dire_cod_sucu = $cod_sucursal and 
                                                asto_cod_ejer = $cod_ejer
                                             
                                ";
            $oIfx->QueryT($sql_insert_dir_an);

            $sql_delete_dir  = "UPDATE saedir set dir_deb_ml=0.00, dir_cre_ml=0.00, 
                                    dir_deb_mex=0.00,dir_cred_mex=0.00
                                    where dire_cod_asto = '$cod_asto' and 
                                    dire_cod_empr = $cod_empresa and 
                                    dire_cod_sucu = $cod_sucursal and 
                                    asto_cod_ejer = $cod_ejer";
            $oIfx->QueryT($sql_delete_dir);


            // ----------------------------------------------------------------------------------------------
            // ELIMINACION DE SAERET
            // ----------------------------------------------------------------------------------------------

            $sql_insert_ret_an  = "INSERT into saeret_an 
                                                select * from 
                                                saeret 
                                                where rete_cod_asto = '$cod_asto' and 
                                                asto_cod_empr = $cod_empresa and 
                                                asto_cod_sucu = $cod_sucursal and 
                                                asto_cod_ejer = $cod_ejer
                                             
                                ";
            $oIfx->QueryT($sql_insert_ret_an);

            $sql_delete_ret  = "DELETE from saeret 
                                    where rete_cod_asto = '$cod_asto' and 
                                    asto_cod_empr = $cod_empresa and 
                                    asto_cod_sucu = $cod_sucursal and 
                                    asto_cod_ejer = $cod_ejer";
            $oIfx->QueryT($sql_delete_ret);
            ...
            // R E C A L C U L O     U L T I M O    C O S T O   C O N   L A     C O M P R A     A N U L A D A

            $sql_fecha_minv = "select minv_fmov from saeminv where minv_num_comp = $num_comp";
            $fecha_minv = consulta_string_func($sql_fecha_minv, 'minv_fmov', $oIfx, '');

            $sql_productos_dmov = "SELECT * from 
                                    saedmov
                                    where 
                                    dmov_num_comp = '$num_comp'";

            if ($oIfx->Query($sql_productos_dmov)) {
                if ($oIfx->NumFilas() > 0) {
                    do {
                        $dmov_cod_prod = $oIfx->f('dmov_cod_prod');

                        $sql_prod_ultim_comp = "SELECT dmov_cod_prod, minv_fmov, minv_hor_minv, dmov_pun_dmov
                                                    from saeminv, saedmov, saedefi
                                                    where 
                                                    dmov_num_comp = minv_num_comp and
                                                    dmov_cod_tran = defi_cod_tran and
                                                    minv_fmov < '$fecha_minv' and
                                                    defi_tip_defi = '0' and
                                                    dmov_cod_prod = '$dmov_cod_prod'
                                                    order by minv_fmov desc, minv_hor_minv desc, dmov_cod_prod
                                                    limit 1;";
                        $dmov_pun_dmov = consulta_string_func($sql_prod_ultim_comp, 'dmov_pun_dmov', $oIfxA, 0);

                        $sql_fecha_tope_nueva_compea = "SELECT minv_fmov from saeminv, saedmov, saedefi
                                                        where 
                                                        dmov_num_comp = minv_num_comp and
                                                        dmov_cod_tran = defi_cod_tran and
                                                        minv_fmov > '$fecha_minv' and
                                                        defi_tip_defi = '0'
                                                        order by minv_fmov;";
                        $dmov_fecha_limite = consulta_string_func($sql_fecha_tope_nueva_compea, 'minv_fmov', $oIfxA, '');
                        if (empty($dmov_fecha_limite)) {
                            $dmov_fecha_limite = date('Y-m-d');
                        }

                        // -------------------------------------------------------------------------
                        // Realizamos el update con la informacion que tenemos
                        // -------------------------------------------------------------------------

                        $sql = "update saedmov set dmov_pun_dmov = $dmov_pun_dmov 
                                    from saeminv
                                    where
                                    minv_num_comp = dmov_num_comp and
                                    dmov_cod_empr = $cod_empresa and
                                    dmov_cod_sucu = $cod_sucursal and
                                    minv_fmov BETWEEN '$fecha_minv' and '$dmov_fecha_limite' and
                                    dmov_cod_prod = '$dmov_cod_prod'	
                                ";
                        $oIfxA->QueryT($sql);
                    } while ($oIfx->SiguienteRegistro());
            ...
            // F I N    R E C A L C U L O     U L T I M O    C O S T O   C O N   L A     C O M P R A     A N U L A D A



            $sql_insert_dmov_an  = "INSERT into saedmov_an 
                                                select * from 
                                                saedmov
									            where 
									            dmov_num_comp = '$num_comp'
                                ";
            $oIfx->QueryT($sql_insert_dmov_an);
            ...
            // ----------------------------------------------------------------------------------------------
            // ELIMINACION DE SAEMXFP
            // ----------------------------------------------------------------------------------------------

            $sql_insert_mxfp_an  = "INSERT into saemxfp_an 
                                                select * from 
                                                saemxfp where mxfp_num_comp = (
                                                                    select minv_num_comp 
                                                                        from saeminv 
                                                                        where minv_comp_cont = '$cod_asto' and
                                                                        minv_cod_empr = $cod_empresa and
                                                                        minv_cod_sucu = $cod_sucursal and
                                                                        minv_cod_ejer = $cod_ejer
                                                                )
                                ";
            $oIfx->QueryT($sql_insert_mxfp_an);

            $sql_delete_mxfp  = "DELETE from saemxfp where mxfp_num_comp = (
				select minv_num_comp 
                    from saeminv 
                    where minv_comp_cont = '$cod_asto' and
                    minv_cod_empr = $cod_empresa and
            ...
            // ----------------------------------------------------------------------------------------------
            // ELIMINACION DE SAEASTO
            // ----------------------------------------------------------------------------------------------

            $sql_insert_minv_an  = "INSERT into saeasto_an 
                                                select * from 
                                                saeasto 
                                                where asto_cod_asto = '$cod_asto' and
                                                asto_cod_empr = $cod_empresa and
                                                asto_cod_sucu = $cod_sucursal and 
                                                asto_cod_ejer = $cod_ejer
                                ";
            ...
            $sql_delete_asto  = "UPDATE saeasto set 
                                    asto_est_asto = 'AN',asto_vat_asto = 0.00,
                                    asto_usu_modi = $id_usuario, asto_fec_modi = '$fecha_actual'

                                    where asto_cod_asto = '$cod_asto' and
                                    asto_cod_empr = $cod_empresa and
                                    asto_cod_sucu = $cod_sucursal and 
                                    asto_cod_ejer = $cod_ejer";
            $oIfx->QueryT($sql_delete_asto);
        ...
        if ($minv_num_comp_oc != 0) {
            ...
            // ACTUALIZAMOS EL ESTADO DE NUESTRA ORDEN DE COMPRA ATADA A ESTE MOVIMEINTO A 'N' DE ESTA FORMA QUEDA PENDIENTE
            $sql = "UPDATE saeminv set minv_cer_sn = 'N' where
                                            minv_num_comp = $minv_num_comp_oc
                                    	";
            $oIfx->QueryT($sql);

            // CONSULTAMOS Y ACTUALIZAMOS LOS PRODUCTOS DE LA ORDEN DE COMPRA PARA ACTUALIZAR LAS CANTIDADES QUE HEMOS UTILIZADO EN ESTA ANULACION
            // (dmov_can_entr)
            $sql_productos_compra_oc = "SELECT dmov_cod_prod, dmov_can_dmov, dmov_can_entr from saedmov where dmov_num_comp = '$minv_num_comp_oc'";
            if ($oIfx->Query($sql_productos_compra_oc)) {
                if ($oIfx->NumFilas() > 0) {
                    do {
                        ...
                        foreach ($array_productos_compra as $key => $productos_compra) {
                            if ($productos_compra['cod_prod'] == $dmov_cod_prod) {
                                $cant_nuev_entr = $dmov_can_entr - $productos_compra['can_prod'];
                                $sql = "UPDATE saedmov set dmov_can_entr = $cant_nuev_entr where
                                            dmov_num_comp = $minv_num_comp_oc and
                                            dmov_cod_prod = '$dmov_cod_prod'
                                    	";
                                $oIfxA->QueryT($sql);
                            }
        ...
        if ($act_inv == 'S') {
            ...
            $select_bodegas = "SELECT bode_cod_bode, bode_nom_bode from saebode, saesubo
                                        where
                                        bode_cod_bode = subo_cod_bode
                                        and subo_cod_sucu = $sucursal
                                        and subo_cod_empr = $empresa
                                        ;";

            if ($oIfx->Query($select_bodegas)) {
                if ($oIfx->NumFilas() > 0) {
                    do {
                        $bode_cod_bode = $oIfx->f('bode_cod_bode');

                        $sql = "  SELECT saeprbo.prbo_cod_prod,   saeprbo.prbo_cod_bode,   saeprbo.prbo_cod_unid,   saeprbo.prbo_dis_prod,   
                                            a.prod_cod_prod,         a.prod_nom_prod,         
                                        (select sum(case d.defi_tip_defi when '0' then c.dmov_can_dmov when '1' then - c.dmov_can_dmov when '5' then c.dmov_can_dmov when '6' then -c.dmov_can_dmov end)
                                                    ...
                                                    b.minv_cod_empr 	=   $empresa and
                                                    (c.dmov_cod_bode   =   $bode_cod_bode) and
                                                    b.minv_fmov       <= CURRENT_DATE )	disponible, 
                                                (select sum(case d.defi_tip_defi when '0' then c.dmov_can_dmov when '1' then - c.dmov_can_dmov when '5' then c.dmov_can_dmov when '6' then c.dmov_can_dmov end)
                                                    from  saedmov c,saeminv b,saedefi d where 
                                                    b.minv_cod_empr	=	c.dmov_cod_empr and 
                                                    b.minv_cod_sucu	=	c.dmov_cod_sucu and
                                                    ...
                                                    d.defi_cod_tran 	= 	b.minv_cod_tran and
                                                    b.minv_cod_empr 	= 	$empresa and
                                                    (c.dmov_bod_envi 	= 	$bode_cod_bode ) and
                                                    b.minv_fmov       <= 	CURRENT_DATE )	destino
                                    FROM saeprbo,   
                                        saeprod a
                                    WHERE ( a.prod_cod_prod 			= saeprbo.prbo_cod_prod ) and  
                                        ...
                                        ( saeprbo.prbo_cod_empr  = $empresa ) AND  
                                        ( saeprbo.prbo_cod_sucu    = $sucursal ) AND  
                                        ( saeprbo.prbo_cod_bode    = $bode_cod_bode )  ";
                        ...
                        if ($oIfxA->Query($sql)) {
                            if ($oIfxA->NumFilas() > 0) {
                                do {
                                    $prod_cod_prod   = $oIfxA->f('prbo_cod_prod');
                                    $prbo_dis_prod   = vacios($oIfxA->f('prbo_dis_prod'), 0);
                                    $disponible      = vacios($oIfxA->f('disponible'), 0);
                                    $destino         = vacios($oIfxA->f('destino'), 0);
                                    $stock_real      = $disponible + $destino;
                                    $diferencia      = 0;
                                    $diferencia      = $prbo_dis_prod - $stock_real;
                                    if ($diferencia != 0) {
                                        $array_prod[] = array($prod_cod_prod, $bode_cod_bode, $stock_real);
                                    }
                                } while ($oIfxA->SiguienteRegistro());
                            }
                        }
            ...
            if (!empty($array_prod)) {
                foreach ($array_prod as $val) {
                    ...
                    // SSAEPRBO
                    $sql = "update saeprbo set prbo_dis_prod = $stock_real where
                                    prbo_cod_empr = $empresa and
                                    prbo_cod_sucu = $sucursal and
                                    prbo_cod_bode = $bode_cod_bode and
...
function agrega_modifica_grid_fp($nTipo = 0, $secuencial = '', $aForm = '', $id = '')
{
    ...
    if (!empty($minv_cod_edit)) {
        ...
        // -------------------------------------------------------------------------------------------------------------------
        // Cargar retenciones de la compra
        // -------------------------------------------------------------------------------------------------------------------

        $ret_num = $aForm['ret_num'];

        unset($_SESSION['aDataGirdRete']);
        $aDataGrid_ret = $_SESSION['aDataGirdRete'];
        $aLabelGrid_ret = array('Id', 'Codigo Sri', 'Porcentaje', 'Base Imponible', 'Retencion', 'No Retencion', 'Cta Contable', 'Eliminar');

        $sql_retencion_minv = "SELECT * from saeminv where minv_num_comp = $minv_cod_edit;";
        if ($oIfx->Query($sql_retencion_minv)) {
            if ($oIfx->NumFilas() > 0) {
                do {
                    $minv_comp_cont = $oIfx->f('minv_comp_cont');
                    $minv_fmov = $oIfx->f('minv_fmov');
                    ...
                    $sql_retencion = "SELECT * from saeret where rete_cod_asto = '$minv_comp_cont' and asto_cod_ejer = $idejer and asto_num_prdo = $idprdo and asto_cod_sucu = $minv_cod_sucu and asto_cod_empr = $minv_cod_empr;";
                    if ($oIfxA->Query($sql_retencion)) {
                        if ($oIfxA->NumFilas() > 0) {
                            do {
                                $ret_num_ret = $oIfxA->f('ret_num_ret');
                                ...
                                //$ret_num_ret = str_pad($ret_num_ret, 9, "0", STR_PAD_LEFT);
                                $oReturn->assign("ret_num", "value", $ret_num_ret);
                                $oReturn->assign("electronica", "value", 'N');
                                $oReturn->assign("ret_num", "value", $ret_num_ret);
                                $oReturn->assign("ret_num", "value", $ret_num_ret);



                                $sql_cuen_ad = "select  tret_cta_cre 
                                        from saetret, saecuen where
                                        tret_cta_cre = cuen_cod_cuen and
                                        tret_cod_empr = $empresa and
                                        tret_cod = '$ret_cta_ret' ";
                                $cuen_ret = consulta_string($sql_cuen_ad, 'tret_cta_cre', $oIfxB, 0);



                                //GUARDA LOS DATOS DEL DETALLE
                                $cont = count($aDataGrid_ret);
                                $fu->AgregarCampoNumerico($cont . '_retencion', 'Retencion|left', true, $ret_num_ret, 70, 10);
                                //$fu->AgregarCampoNumerico($cont . '_retencion', 'Retencion|left', true, $ret_num, 70, 10);
                                $aDataGrid_ret[$cont][$aLabelGrid_ret[0]] = floatval($cont);
                                $aDataGrid_ret[$cont][$aLabelGrid_ret[1]] = $ret_cta_ret;
                                $aDataGrid_ret[$cont][$aLabelGrid_ret[2]] = $ret_porc_ret;
                                $aDataGrid_ret[$cont][$aLabelGrid_ret[3]] = $ret_bas_imp;
                                $aDataGrid_ret[$cont][$aLabelGrid_ret[4]] = round(($ret_bas_imp * $ret_porc_ret / 100), 2);
                                $aDataGrid_ret[$cont][$aLabelGrid_ret[5]] = $fu->ObjetoHtml($cont . '_retencion'); //retencion;
                                $aDataGrid_ret[$cont][$aLabelGrid_ret[6]] = $cuen_ret;            // cuenta;
                                $aDataGrid_ret[$cont][$aLabelGrid_ret[7]] = '<img src="' . $_COOKIE['JIREH_IMAGENES'] . 'iconos/delete_1.png"
																						onMouseOver="drc(\'Presione aqui para Eliminar\', \'Eliminar\'); return true;"
                                ...
                                $_SESSION['aDataGirdRete'] = $aDataGrid_ret;
                                $sHtml = mostrar_grid_ret();
                                $oReturn->assign("divFormularioCabeceraRET", "innerHTML", $sHtml);
        ...
        // -------------------------------------------------------------------------------------------------------------------
        // FIN Cargar retenciones de la compra
        // -------------------------------------------------------------------------------------------------------------------
...
function tipo_factura($aForm = '')
{
    ...
    if ($cliente != '') {
        ...
        $minv_cod_edit = $_SESSION['num_comp_edit'];
        if (!empty($minv_cod_edit)) {
            $oReturn->script("cargar_info_fact_ad($minv_cod_edit)");
        }
        // --------------------------------------------------------------------------------------
        // FIN cargar compra de inventario cuando se vaya a modificar
        // --------------------------------------------------------------------------------------
    ...
    try {
        ...
        if ($estado[$pos] == 'AUTORIZADO') {
            ...
            if (!empty($informacionAdicional)) {
                ...
                $mensaje = 'La Clave de Acceso ingresada no pudo ser consultada, compruebe la información en el SRI';
            }
...
function recalcular_precio_venta($nomp_cod_nomp, $aForm = '')
{
    ...
    $calculo_precio = number_format($costo_prod_tot + $calculo_porcentaje, 4, '.', '');
