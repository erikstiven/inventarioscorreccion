...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script>
            function datos( a,b,c,d,e){
                ...
                // Adrian asignar valor si es inva o subtotal
                window.opener.asignarValorIvaSub(e);
    ...
    <body>
        ...
        $sql = "select tret_cod, tret_det_ret, tret_porct, 
                    tret_cta_deb, tret_cta_cre, tret_ban_crdb, tret_imp_ret
                    from saetret where
        ...
        if ($oIfx->Query($sql)) {
            if( $oIfx->NumFilas() > 0 ) {
                do {
                    ...
                    $tret_imp_ret        = $oIfx->f('tret_imp_ret');
                    ...
                    <a href="#" onclick="datos('<? echo $cod_ret;?>',    '<? echo $porc_ret;?>',    '<? echo $cta_deb ?>',      
                                               '<? echo $cta_cre ?>',    '<? echo $tret_imp_ret ?>' )">
                         <? echo $cod_ret;?></a>
                    <?
                    echo '</td>';
                    echo '<td>'
